...
public interface CarriageQueueRepository extends JpaRepository<CarriageQueue, Long> {
   ...
   * 获取指定联盟当前最大的排队顺序号
   */
  @Query("SELECT COALESCE(MAX(cq.queueOrder), 0) FROM CarriageQueue cq WHERE cq.allianceId = :allianceId")
  ...
  void decrementQueueOrderAfter(@Param("allianceId") Long allianceId, @Param("queueOrder") Integer queueOrder);
   ...
   */
  List<CarriageQueue> findByAllianceIdAndLastDriveDateNotNullAndLastDriveDateNot(Long allianceId, LocalDate date);
